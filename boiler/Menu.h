...
uint16_t Goto_Menu (uint8_t *array_Byte, uint8_t *type)
{
	...
	int h=2500;																														//Счетчик для принудительного выхода из меню (25с)
	while(1)
	{
		h--;																														//Декрементируем счетчик, если не происходит нажатие на кнопки
		if (h==1)																													//Если в меню, контроллер находится в бездействии 25с
		{
			//Обнуляем пароль
			Password_s[0] = '0';
			Password_s[1] = '0';
...
//------------------------------------------------- Обработка кнопки 0 -------------------------------------------------------------//
		...
		{
			h=2500;																													//При нажатии на кнопку обновляем счетчик
			...
			while(Key(0) == 1)
			{
				...
				if (_my_Lib_Flags & lib_Flag_Changing_Param)
				{
					...
					else
					{
						...
						LED_Clear();																									
						LED_SetPos(0, 0);																								
						LED_Write_String_P(PSTR("Неправильный"));
						LED_SetPos(1, 0);
						LED_Write_String_P(PSTR("пароль"));
						_delay_ms(2000);
					}
				}
